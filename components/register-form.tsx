...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      // 分离Base64数据的MIME类型和数据部分
      let logoData = formData.logo;
      let logoMimeType = 'image/png';

      if (logoData && logoData.includes('data:')) {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || '注册失败，请稍后重试');
      }

      // 注册成功 - API返回 { success: true, id, name, message }
      setSuccessMessage(`注册成功！您的机构ID是: ${data.id}。请保存此ID用于后续操作。`);
