...
async function extractQuestionsWithVision(
  ...
  const prompt = `你是一个专业的物理试卷识别专家。请从图片中识别并提取所有题目。
  ...
  "questions": [
    {
      ...
      "content": "题干内容（不包括选项）",
      "type": "choice",
      "options": ["选项A完整内容", "选项B完整内容", "选项C完整内容", "选项D完整内容"]
    }
  ]
}

**注意事项：**
1. 只返回纯 JSON，不要有 markdown 代码块
2. 必须识别出 7 道题
3. A. B. C. D. 是选项，不是独立题目
4. 如果有年份信息如 "(2011·江苏·4)"，这是题目来源，不是题目编号`;
